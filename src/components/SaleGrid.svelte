<script>
  ...
  let load = function () {
    fetch("https://catalog.klotz-imbiss.de/api/card?populate=angebot")
      .then((response) => {
        return response.json();
      })
      .then((data) => {
        items = data.data.attributes.angebot.data;
      });
  };
...
<h2 class="font-bold mt-6 mb-3 text-white">Angebot der Woche</h2>
