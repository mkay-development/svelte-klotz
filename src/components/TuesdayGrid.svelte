<script>
  ...
  let load = function () {
    fetch("https://catalog.klotz-imbiss.de/api/card?populate=dienstag")
      .then((response) => {
        return response.json();
      })
      .then((data) => {
        items = data.data.attributes.dienstag.data;
      });
  };
...
<h2 class="mt-6 mb-3 text-white font-bold">Dienstag</h2>
