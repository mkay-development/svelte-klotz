<script>
  ...
  let load = function () {
    fetch("https://catalog.klotz-imbiss.de/api/card?populate=donnerstag")
      .then((response) => {
        return response.json();
      })
      .then((data) => {
        items = data.data.attributes.donnerstag.data;
      });
  };
...
<h2 class="mt-6 mb-3 text-white font-bold">Donnerstag</h2>
